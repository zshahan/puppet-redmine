...
define redmine::instance (
        ...
        'mysql' : {
          if !defined(Package['libmysql++-dev']) {
            package { 'libmysql++-dev':
              ...
              before => Exec["redmine-bundle-${name}"],
            }
          }
  ...
  exec { "redmine-migrate-${name}":
    ...
    require => [Exec["redmine-bundle-${name}"], File["${app_root}/current/config/database.yml"]],
    notify  => Service["${name}"],
    user    => $redmine_user,
    group   => $redmine_user,
    timeout => 600,
  ...
  exec { "redmine-configure-${name}":
    require => Exec["redmine-migrate-${name}"],
    path    => "/bin:/usr/bin",
